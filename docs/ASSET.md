...
| `name`        | Alternative to `key` for an asset | When checking what items are in a room, or in your inventory the `key` will be used by default. However you may wish an item or location to be given a richer of more readable name, so you could have a file called 'bananas.yml' with a `name` 'bunch of bananas'. The game allows both the `name` and the `key` to be used in commands e.g. "take bananas" or "take bunch of bananas" will both work. | String: [front_door_key.yaml#L1](../samples/sample-game/assets/front_door_key.yaml#L1) |
| `description` | Long description for an asset | In the game more information about an item can be obtained by using the "describe" command; or for a location the "look" command or by visiting a location for the first time. In both situations the `description` is what will be shown to the user | String: [earthsea.yaml#L1](../samples/sample-game/assets/earthsea.yaml#L1), Array of Strings: [bathroom.yaml#L1](../samples/sample-game/assets/bathroom.yaml#L1), `when` Object: [bottle.yaml](../samples/sample-game/assets/bottle.yaml#L1) |
| `items`       | List of assets in a location | For any items that you want to link to the location, use its `key` (_not_ the `name`). These will be listed against the location when it is described | Array of Strings: [kitchen.yaml#L5](../samples/sample-game/assets/kitchen.yaml#L5) |
| `takeable`    | Whether an item can be taken | By default, any item in a room can be taken. However, to prevent an item being taken the `takeable` property can be set. If this is set to `false` then a standard message will be used if a player tries to take it; if it is a String then that message will be shown instead | Boolean: [kitchen_fridge.yaml#L2](../samples/sample-game/assets/kitchen_fridge.yaml#L2), or String: [photograph.yaml](../samples/sample-game/assets/photograph.yaml#L2) |
| `actions`     | Actions that can be done to an item or location | These are complex objects described in a separate section below | Array of `action` objects: [bottle.yaml#L9](../samples/sample-game/assets/bottle.yaml#L9) |
...
| Type | Example |
| ---- | ------- |
| always          | [hall.yaml#L3](../samples/sample-game/assets/hall.yaml#L3)         |
| state check     | [hall.yaml#L5](../samples/sample-game/assets/hall.yaml#L5)         |
| inventory check | [earthsea.yaml](../samples/sample-game/assets/earthsea.yaml#L5)    |
| state and inventory check | [hall.yaml#L70](../samples/sample-game/assets/hall.yaml#L70) |
| state but not inventory check | [hall.yaml#L73](../samples/sample-game/assets/hall.yaml#L73) |
| not state and not inventory check | [kitchen_drawers.yaml#L12](../samples/sample-game/assets/kitchen_drawers.yaml#L12) |
| mixed inventory check         | [kitchen_cupboard.yaml#L14](../samples/sample-game/assets/kitchen_cupboard.yaml#L14) |
| else check      | [kitchen_drawers.yaml#L12](../samples/sample-game/assets/kitchen_drawers.yaml#L27) |
